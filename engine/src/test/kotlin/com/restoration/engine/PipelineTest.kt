...
class PipelineTest {
    @Test
    fun `planner adds deblock for high artifacts`() {
    ...
    @Test
    fun `planner skips SR for high-res`() {
        val planner = RuleBasedPipelinePlanner()
    ...
    @Test
    fun `pipeline emits progress and completes`() = runTest {
        val pipeline = RestorationPipeline()
        val r = RestoreRequest(input = ImageInput(path = "test.jpg"))
