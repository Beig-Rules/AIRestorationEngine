...
class RestorationPipeline(
    ...
    private val backendSelector: () -> BackendSelector = {
        BackendSelector(listOf(CpuBackend()))
    }
) {
    constructor(
        planner: RuleBasedPipelinePlanner,
        selector: BackendSelector
    ) : this(planner, { selector })

    suspend fun execute(request: RestoreRequest): Flow<com.restoration.engine.RestoreProgress> =
        flow {
            ...
            val quality = withContext(Dispatchers.Default) { analyzeQuality(request.input) }
            ...
            val stageCount = planned.stages.size.coerceAtLeast(1)
            for ((idx, stage) in planned.stages.withIndex()) {
                if (!coroutineContext.isActive) {
                    emit(com.restoration.engine.RestoreProgress.Completed(Result.failure(EngineError.Cancelled)))
                    return@flow
                }
                emit(com.restoration.engine.RestoreProgress.StageStarted(stage, idx.toFloat() / stageCount))
                try {
                    val outPath = request.input.path.replace(Regex("\\.[^.]+$"), "_${stage.lowercase()}.tmp")
                    currentQuality = withContext(Dispatchers.Default) {
                        backend.runStage(stage, request.input.path, outPath, request.targetScale, currentQuality)
                    }
                    stagesDone++
